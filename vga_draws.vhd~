...
signal s_ghost_ARGB      : VGA_ARGB_t;
...
begin
	...
	ghost_draws : entity work.ghost_draws(struct)
	port map(map_x => s_map_x,
	         map_y => s_map_y,
				ghost_x => ghost_x,
				ghost_y => ghost_y,
				texture_id  => ghost_tex_id,
				ARGB        => s_ghost_ARGB);
	process (s_in_map, s_pac_ARGB, s_map_ARGB, s_scores_ARGB, s_energyzers_ARGB, s_ghost_ARGB)
	begin
		RGB <= "000";
		...
		if ( s_in_map = '1' ) then
			...
			elsif ( s_ghost_ARGB(3) = '1' ) then
				RGB <= s_ghost_ARGB(2 downto 0);
